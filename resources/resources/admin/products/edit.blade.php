        ...
        <div class="card-body">
            <form action="" method="post" class="row">
                ...
                <div class="col-6 my-2">
                    ...
                    <input type="number" name="coin" class="form-control" value="{{ $product->coin }}">
                    @error('coin')
                    <label class="text-danger">{{ $message }}</label>
                ...
                <div class="col-12 my-2">
                    ...
                    <select name="parent_id" id="" class="form-select">
                        ...
                        @foreach ($sub_categories as $item)
                            <option value="{{ $item->id }}" @selected($item->id == $product->parent_id)>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('parent_id')
                    <label class="text-danger">{{ $message }}</label>
                ...
                <div class="col-3 my-3"><button class="btn btn-primary" type="submit">Cập nhật</button></div>
            </form>
        </div>
    </div>
</div>
